...
from librarySystem.settings import BASE_DIR
...
def user(request, pk_test):
    ...
    context = {'user': user, 'borrows_count': borrows_count,
               'borrows': borrows, 'myFilter': myFilter}
    return render(request, 'accounts/user.html', context)


def createBorrow(request, pk):
    BorrowFormSet = inlineformset_factory(
        User, Borrowed, fields=('book', 'status'),)
...
def recommender(request):
    dataset = pd.read_csv(BASE_DIR+'\\datasets\\ratings.csv')
    ...
    if os.path.exists('regression_model2.h5'):
        ...
        history = model2.fit([train.userID, train.bookID],
    ...
    user = np.array([n[random.randint(0, 1000)]
    ...
    books = pd.read_csv(BASE_DIR+'\\datasets\\books.csv')
    #values = {}
    ...
    for ids in user_input:
        for index, row in books.iterrows():
            if row["id"] == user_input[counter]:
                list_of_recommend.append(
                    recommend(row["id"], row["title"], row["author"], row["language"]))
        counter = counter + 1
    ...
    context = {'list_of_recommend': list_of_recommend, 'booklength': booklength,
               'countScore': countScore, 'predictionMaxLength': predictionMaxLength}
